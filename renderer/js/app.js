...
function initCosmos() {
  ...
  let W, H, stars = [], meteors = [], raf, lastFrame = 0;
  const TARGET_FPS = 20; // was 60fps — GPU was burning for nothing
  ...
  function spawnStars(n = 90) { // was 180
  ...
  // Pause canvas AND all CSS animations when window is hidden/minimized
...
async function doConnect() {
  ...
  try {
    ...
    log('info', `xray listening on HTTP :${res.httpPort} SOCKS :${res.socksPort}`);
    renderSrvList(); renderQs();

    // Check real external IP through proxy (non-blocking)
    try {
      const ipRes = await window.spinerAPI.getExternalIp();
      if (ipRes && ipRes.ok) {
        log('info', `External IP: ${ipRes.ip}  (${ipRes.country}, ${ipRes.isp})`);
        toast(`IP: ${ipRes.ip} — ${ipRes.country}`, 'success');
        // Show in status bar if element exists
        const ipEl = document.getElementById('externalIp');
        if (ipEl) ipEl.textContent = ipRes.ip;
      } else {
        log('warn', `Could not fetch external IP: ${ipRes ? ipRes.error : 'no response'}`);
      }
    } catch (ipErr) {
      log('warn', 'IP check failed: ' + ipErr.message);
    }
  } catch (e) {
    S.connecting = false;
    setUI('disconnected');
